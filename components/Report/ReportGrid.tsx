  ...
  return (
    <section className="w-full h-[calc(100vh-118px)] bg-white-main border-[1px] rounded-[8px] shadow-secondary">
      ...
      <div className="w-full h-[calc(100%-120px)]">
        <div
          className={`w-full h-[40px] grid ${GridCols} min-w-[800px] border-b-[1px] border-gray px-4`}
        >
          {TableHeaders.map((item: string, index: any) => (
            <div
              key={index}
              className="w-full flex items-center justify-start capitalize font-medium whitespace-nowrap text-black-main text-[14px] font-inter"
            >
              {item}
            </div>
          ))}
